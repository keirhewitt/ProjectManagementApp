...
class RoomCreationForm(FlaskForm):
    ...
    room_name = StringField(validators=[DataRequired(), Length(
        min=4, max=40
    )], render_kw={"placeholder": "Room name"})
...
class TaskCreationForm(FlaskForm):
    ...
    project = StringField(validators=[DataRequired(), Length(
        min=1, max=60
    )], render_kw={"placeholder": "Project"})

    job = StringField(validators=[DataRequired(), Length(
        min=1, max=60
    )], render_kw={"placeholder": "Job"})

    assignee = StringField(validators=[DataRequired(), Length(
        min=1, max=60
    )], render_kw={"placeholder": "Assignee"})

    progress = StringField(validators=[DataRequired(), Length(
        min=1, max=60
